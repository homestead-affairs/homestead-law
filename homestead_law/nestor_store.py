...
``<household_root>/nestor-law.db``. It is the host's adapter — distinct from
``nestor_seam.py``, which is the boundary. The seam says *how* this module
calls Nestor; this module says *where* the memory lives.
...
def store_path(household_root: Path | None = None) -> Path:
    """The path where Nestor's law store lives — inside the household root."""
    root = Path(household_root) if household_root is not None else paths.home()
    return root / "nestor-law.db"


def get_store(household_root: Path | None = None) -> Any:
    """A SqliteStore at ``<household_root>/nestor-law.db``, cached per process.
