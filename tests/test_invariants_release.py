...
Ported into homestead-law from the ledger (homestead-ledger), which ported it
from the engine (homestead-affairs), which ported it from jeles and willow-mcp,
where those four were paid for. This is law's first PyPI wiring, so these are
guards against a first-time miswire and a future edit, not a live bug.

**Ported, not copied.** The checks that differ by repo are made to fail if a
constant was carried over unchanged: homestead-law is below 1.0 and packages
`homestead_law/`, not the engine's `homestead/`, the ledger's
`homestead_ledger/`, or willow-mcp's `src/willow_mcp/`; and it stores no
version literal anywhere, because the tag is the only source. Getting those
backwards is exactly the copy-paste failure the wiring test exists to catch.

homestead-law has no `packaging/` directory and no PyInstaller `artifact` CI
job — unlike the ledger, it is not shipped as a double-clickable binary here.
That is a separate concern from the PyPI release chain this file guards, so
there is deliberately no assertion in this file about a packaging spec or an
artifact job; ci.yml's aggregate `test` job needs only `[invariants,
release-wiring]`.
"""
...
def test_the_version_has_exactly_one_source():
    ...
    hardcoded = [
        ...
        for p in (_REPO / "homestead_law").rglob("*.py")
...
def test_the_pr_title_check_guards_both_directions():
    ...
    layout, `homestead_ledger/` is the ledger's, and `src/willow_mcp/` is
    willow-mcp's; here the wheel packages `homestead_law`, so a copied constant
    would make the check pass on everything."""
    wf = _REPO / ".github" / "workflows" / "pr-title.yml"
    body = _yaml(wf)["jobs"]["title"]["steps"][-1]["run"]
    assert 'PACKAGED = ("homestead_law/", "pyproject.toml")' in body, \
        "packaged path is wrong or was copied from another repo"
    assert 'PACKAGED = ("homestead/"' not in body, "the engine's path leaked in"
    assert 'PACKAGED = ("homestead_ledger/"' not in body, "the ledger's path leaked in"
    ...
    pyproject = tomllib.loads((_REPO / "pyproject.toml").read_text())
    assert pyproject["tool"]["hatch"]["build"]["targets"]["wheel"]["packages"] == ["homestead_law"]
...
def test_ci_wiring_has_no_artifact_leg():
    """homestead-law has no `packaging/` directory and no PyInstaller artifact
    build — unlike the ledger, this module is not shipped as a double-clickable
    binary. The aggregate `test` job must gate on exactly the legs that exist
    here, not on a job this repo never defines: a `needs:` entry naming a
    nonexistent job makes the workflow fail to parse at all, and copying the
    ledger's three-way gate verbatim would do exactly that."""
    ci = _yaml(_REPO / ".github" / "workflows" / "ci.yml")
    jobs = ci["jobs"]
    assert "artifact" not in jobs, (
        "no packaging/ dir and no PyInstaller spec exist here — an artifact "
        "job would build nothing real"
    )
    assert set(jobs["test"]["needs"]) == {"invariants", "release-wiring"}, (
        jobs["test"]["needs"]
    )
